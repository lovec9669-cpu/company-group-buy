...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { data: group, error: findError } = await supabase.from("group_buys").select("id,status,end_at").eq("id", id).single();
    ...
    if (group.status === "reviewing" && transition.next === "awaiting_payment") {
      ...
      const { data: orders, error: orderError } = await supabase.from("orders").select("id").eq("group_buy_id", id);
      if (orderError) throw orderError;
      const orderIds = (orders ?? []).map((order) => order.id);
      if (orderIds.length) {
        const { data: items, error: itemError } = await supabase.from("order_items").select("id,product_id,quantity").in("order_id", orderIds);
        ...
        for (const item of items ?? []) {
          ...
          const { error: updateError } = await supabase.from("order_items").update({ final_quantity: quantity, final_unit_price: unitPrice, final_amount: amount }).eq("id", item.id);
