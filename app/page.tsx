...
export default function Home() {
  ...
  async function loadGroups() { try { const response = await fetch("/api/group-buys", { cache: "no-store" }); const result = await response.json(); if (response.ok) setGroups(result.data ?? []); } finally { setLoading(false); } }
  ...
  return (
    <main className="min-h-screen px-5 py-8 md:px-10"><div className="mx-auto max-w-5xl">
      ...
      <section className="mb-10"><div className="mb-4"><h2 className="text-xl font-bold">進行中的團購</h2><p className="mt-1 text-sm text-[var(--muted)]">選擇團購後即可查看商品與目前價格。</p></div>{loading ? <div className="rounded-3xl border border-[var(--border)] bg-white p-8 text-center text-sm text-[var(--muted)]">正在讀取團購...</div> : openGroups.length === 0 ? <div className="rounded-3xl border border-dashed border-[var(--border)] bg-white p-8 text-center text-sm text-[var(--muted)]">目前沒有進行中的團購。</div> : <div className="grid gap-4 md:grid-cols-2">{openGroups.map((group) => <article key={group.id} className="rounded-3xl border border-[var(--border)] bg-white p-6 shadow-sm"><span className="inline-flex rounded-full bg-[#e8f3ef] px-3 py-1 text-xs font-semibold text-[var(--accent)]">訂購中</span><h3 className="mt-3 text-xl font-bold">{group.name}</h3>{group.description && <p className="mt-2 text-sm text-[var(--muted)]">{group.description}</p>}<div className="mt-5 space-y-2 text-sm text-[var(--muted)]"><p>開始：{formatDate(group.start_at)}</p><p>截止：{formatDate(group.end_at)}</p></div><button className="mt-6 w-full rounded-xl bg-[var(--accent)] px-4 py-3 font-semibold text-white">進入團購</button></article>)}</div>}</section>
