...
export default function PaymentsPage() {
  ...
  async function setPaid(groupId: string, orderId: string, paid: boolean) {
    setBusyId(orderId);
    setError("");
    setMessage("");
    try {
      const response = await fetch("/api/admin/payments", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "member", orderId, paid }) });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error ?? "更新付款狀態失敗");
      setMessage(result.message ?? "付款狀態已更新");
      await load();
      setExpandedId(groupId);
    } catch (e) {
      setError(e instanceof Error ? e.message : "更新付款狀態失敗");
    } finally {
      setBusyId(null);
    }
  }
  ...
  async function finalizeGroup(group: PaymentGroup) {
    if (group.paidCount !== group.memberCount) {
      setError("仍有人尚未付款，請先把所有人的開關切到 ON。");
      return;
    }
    ...
    <main className="min-h-screen px-5 py-8 md:px-10">
      <div className="mx-auto max-w-6xl">
        <header className="mb-8 rounded-3xl bg-white p-6 shadow-sm">
          ...
          <p className="mt-2 text-sm text-[var(--muted)]">這裡顯示已發布結果的團購。每位團員會顯示這一次團購的應收總金額，付款狀態預設都是 OFF，由管理員逐一切換。</p>
        </header>
        ...
        {loading ? (
          ...
          <section className="space-y-4">
            {groups.map((group) => {
              ...
              const allPaid = group.memberCount > 0 && group.paidCount === group.memberCount;
              return (
                <article key={group.id} className="overflow-hidden rounded-3xl bg-white shadow-sm">
                  <button type="button" onClick={() => setExpandedId(expanded ? null : group.id)} className="flex w-full items-center justify-between gap-4 p-6 text-left hover:bg-[#fafbf9]">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-[var(--muted)]">結束時間：{formatDate(group.end_at)} · 已付款 {group.paidCount}/{group.memberCount} 人</p>
                    </div>
                  ...
                  {expanded && (
                    <div className="border-t border-[var(--border)] p-6">
                      <div className="mb-5 grid gap-3 md:grid-cols-3">
                        ...
                        <div className="rounded-2xl bg-[#f4f5f1] p-4"><p className="text-xs text-[var(--muted)]">付款進度</p><p className="mt-1 text-lg font-bold">{group.paidCount} / {group.memberCount} 人</p></div>
                      </div>
                        ...
                        {group.members.map((member) => (
                          <div key={member.orderId} className="grid grid-cols-[1fr_180px_150px] items-center border-t border-[var(--border)] px-5 py-4 text-sm">
                            ...
                            <div className="flex justify-end">
                              <button type="button" role="switch" aria-checked={member.paid} disabled={busyId === member.orderId} onClick={() => setPaid(group.id, member.orderId, !member.paid)} className={`relative h-8 w-14 rounded-full transition disabled:opacity-50 ${member.paid ? "bg-[var(--accent)]" : "bg-gray-300"}`}>
                                <span className={`absolute top-1 h-6 w-6 rounded-full bg-white shadow-sm transition ${member.paid ? "left-7" : "left-1"}`} />
                              </button>
                              <span className={`ml-2 w-10 text-xs font-semibold ${member.paid ? "text-[var(--accent)]" : "text-[var(--muted)]"}`}>{member.paid ? "ON" : "OFF"}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="mt-5 flex flex-col items-end gap-3 rounded-2xl bg-[#f4f5f1] p-5">
                        <p className="text-sm text-[var(--muted)]">全部團員付款後，才可以將本筆團購移到歷史團購。</p>
                        <button type="button" disabled={!allPaid || busyId === group.id} onClick={() => finalizeGroup(group)} className="rounded-xl bg-[var(--accent)] px-5 py-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40">
