  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-[var(--border)] bg-white md:flex md:flex-col">
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`block rounded-xl px-4 py-3 text-sm font-medium transition ${active ? "bg-[var(--accent)] text-white" : "text-[var(--text)] hover:bg-[#f4f5f1]"}`}
            >
              {item.label}
            </Link>
          );
      ...
      <div className="mt-auto border-t border-[var(--border)] p-4">
        <Link
          href="/"
          className="block rounded-xl border border-[var(--border)] px-4 py-3 text-center text-sm font-medium text-[var(--text)] transition hover:bg-[#f4f5f1]"
        >
          回首頁
        </Link>
      </div>
